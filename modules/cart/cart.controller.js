...
export const createCartController = async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json(new ApiResponse(cart, "Cart created/retrieved successfully", 201));
  } catch (error) {
...
export const confirmPaymentController = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json(
        new ApiResponse(cart, "Payment confirmed and order completed", 200),
      );
  } catch (error) {
    res.status(error.statusCode || 500).json({
      message: error.message,
...
export const getBillHistoryController = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json(new ApiResponse(bills, "Bill history retrieved successfully", 200));
  } catch (error) {
    res.status(error.statusCode || 500).json({
      message: error.message,
      statusCode: error.statusCode || 500,
